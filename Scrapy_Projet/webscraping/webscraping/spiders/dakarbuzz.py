...
class dakarabuzzSpider(Spider):
    name = "dakarbuzz"
    allowed_domains = ["dakarbuzz.net"]
    start_urls = ["https://www.dakarbuzz.net/"]
...
#collection = database["articles"]  # Change collection name if needed
    ...
    def parse(self, response):
            ...
            links = response.css("h2.penci-entry-title  a::attr(href)").extract()
            items["links"] = links
            for link in links:
                print(link)
    ...
    def parse_items(self, response):
            ...
            Categorie = response.css("span.cat ::text").extract()
            Titre = response.css("div.grid-header-box ::text").extract()
            Contenu = response.css("div.post-entry p::text").extract()
            Commentaire = response.css("span.single-comment-o::text").extract()
